...
/**
 * Covers the two paths of RestrictedStringRenderer that do not need the Fluid engine: the fallback
 * used when parsing failed, and the fallback used when the view cannot be restricted.
 */
#[CoversClass(RestrictedStringRenderer::class)]
#[CoversMethod(RestrictedStringRenderer::class, 'render')]
#[CoversMethod(RestrictedStringRenderer::class, 'removeFluidSyntax')]
#[CoversMethod(RestrictedStringRenderer::class, 'replaceMarkers')]
final class RestrictedStringRendererTest extends UnitTestCase
{
    ...
    /**
     * Dataprovider replaceMarkersReturnsString()
     */
    public static function replaceMarkersReturnsStringDataProvider(): \Iterator
    {
        yield 'marker is replaced' => [
            'Hello {firstname}',
            ['firstname' => 'Max'],
            'Hello Max',
        ];
        yield 'marker value is escaped like fluid would' => [
            'Hello {firstname}',
            ['firstname' => '<b>Max</b>'],
            'Hello &lt;b&gt;Max&lt;/b&gt;',
        ];
        yield 'quotes are escaped' => [
            'Hello {firstname}',
            ['firstname' => 'Ma"x'],
            'Hello Ma&quot;x',
        ];
        yield 'unknown marker is kept' => [
            'Hello {unknown}',
            ['firstname' => 'Max'],
            'Hello {unknown}',
        ];
        yield 'array values are skipped' => [
            'Hello {firstname}',
            ['firstname' => ['Max']],
            'Hello {firstname}',
        ];
    ...
    #[DataProvider('replaceMarkersReturnsStringDataProvider')]
    #[Test]
    public function replaceMarkersReturnsString(string $string, array $variables, string $expectedResult): void
    {
        self::assertSame($expectedResult, $this->subject->_call('replaceMarkers', $string, $variables));
    }
}
